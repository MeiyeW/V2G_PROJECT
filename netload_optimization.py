...
class CentralOptimization(object):
    ...
    def solve(self, project, net_load, real_number_of_vehicle, price=None, vehCap=None,SOC_margin=0.02,
    ...
    def initialize_model(self, project, net_load, SOC_margin, SOC_offset, price,vehCap):
    ...
    def process(self, times, vehicles, d, pmax, pmin, emin, emax, efinal, 
                pr_e, pr_fre_c1, pr_fre_c2, genCap,regupCap,regdownCap, timeindex, re_energy_u,re_energy_d,
        ...
        with SolverFactory(solver) as opt:
            ...
            if peak_shaving == 'economic':
                def objective_rule(model):
                    ...
                    +sum([model.c1[t, v]*model.pr_fre_c1[t]*model.time_index for v in model.v]) # regup energy market revenue
                    +sum([model.c2[t, v]*model.pr_fre_c2[t]*model.time_index for v in model.v]) # reg down energy market revenue
                    ...
                    -sum([(model.u1Positive[t, v]+model.u1Negative[t, v])*120*model.time_index for v in model.v])  # cost of battery degradation in the unit of $/kwh 
                    -sum([model.c1[t, v]*120*model.time_index for v in model.v])
                    -sum([model.c2[t, v]*120*model.time_index for v in model.v])
                    for t in model.t] )
                    #model.pr_ba
...
#             # ###### Variable           
#             model.u1= Var(model.t, model.v,  within=Reals,doc='Power used for energy discharging')
#             model.ub= Var(model.t, model.v,  within=Binary,doc='binary indicator for Power used for energy discharging')
#             # model.u1Positive=Var(model.t, model.v,  within=NonNegativeReals,doc='Power used for energy discharging')
#             # model.u1Negative=Var(model.t, model.v,  within=NonNegativeReals,doc='Power used for energy discharging')
          
#             model.c1= Var(model.t, model.v,  within=NonNegativeReals, doc='Power capacity used for regulation up')
#             model.c2= Var(model.t, model.v,  within=NonNegativeReals,doc='Power capacity used for regulation down')
#             model.cb= Var(model.t, model.v,  within=Binary,doc='binary Power used for energy capacity reg up')
#             model.cb2= Var(model.t, model.v, within=Binary,doc='binary Power used for energy capacity reg down')
...
#             def final_energy_balance_rule(model, v):
...
#             model.final_energy_balance_rule = Constraint(model.v, rule=final_energy_balance_rule, doc='E final rule')

#             def generation_capacity_rule(model, t, v):
#                 return sum(model.u1[t, v]*model.ub[t,v] for v in model.v ) <= model.genCap[t]
#             model.generation_capacity_rule = Constraint(model.t, model.v, rule=generation_capacity_rule, doc='generation Capcity rule')
            
#             def regup_capacity_rule(model, t, v):
#                 return sum(model.c1[t, v]*model.cb[t,v] for v in model.v ) <= model.regupCap[t]
#             model.regup_capacity_rule = Constraint(model.t, model.v, rule=regup_capacity_rule, doc='reg up Capcity rule')
            
#             def regdown_capacity_rule(model, t, v):
#                 return sum(model.c2[t, v]*model.cb2[t,v] for v in model.v ) <= model.regdownCap[t]
#             model.regdown_capacity_rule = Constraint(model.t, model.v, rule=regdown_capacity_rule, doc='regdown Capcity rule')

#             # def funcname(self, parameter_list):
#             #     #u1=u1Positive-u1Negative
#             #     pass
#             # Set the objective to be either peak shaving or ramp mitigation
#             # still have to deal with time step(model.time_index, for example,15/60mins) and battery price
...
#                 def objective_rule(model):
#                     return sum( [
#                     -sum([model.u1[t, v]*model.ub[t,v]*model.pr_e[t]*model.time_index for v in model.v])# charge cost and discharge revenue, u1 is negative for discharge, positive for charge
#                     +sum([model.c1[t, v]*model.cb[t,v]*(model.pr_fre_c1[t])*model.time_index for v in model.v]) # regup energy market revenue
#                     +sum([model.c2[t, v]*model.cb2[t,v]*(model.pr_fre_c2[t])*model.time_index for v in model.v]) # reg down energy market revenue
#                     # +sum([model.c1[t, v]*model.cb[t,v]*model.re_energy_u*model.pr_fre_c1[t]*model.time_index for v in model.v])  # regup capacity market revenue
#                     # +sum([model.c2[t, v]*model.cb2[t,v]*model.re_energy_d*model.pr_fre_c2[t]*model.time_index for v in model.v]) # reg down capacity market revenue
#                     # -sum([(model.u1Positive[t, v]+model.u1Positive[t, v])*model.ub[t,v]*0.120*model.time_index for v in model.v])  # cost of battery degradation in the unit of $/kwh 
#                     # -sum([model.c1[t, v]*model.cb[t,v]*0.00120* model.time_index for v in model.v])
#                     # -sum([model.c2[t, v]*model.cb2[t,v]*0.00120* model.time_index for v in model.v])
#                     for t in model.t] )
#                     #model.pr_ba
#                 model.objective = Objective(rule=objective_rule, sense=maximize, doc='Define objective function')
...
#         p2=pandas.DataFrame(index=['energy binary'], data=ub).transpose()
#         p3=pandas.DataFrame(index=['regup'], data=c1).transpose()
#         p4=pandas.DataFrame(index=['regup binary'], data=cb).transpose()

#         p5=pandas.DataFrame(index=['regdown'], data=c2).transpose()
#         p6=pandas.DataFrame(index=['regdown binary'], data=cb2).transpose()

#         result_pd=pandas.concat([p1_charge,p1_discharge,p2,p3,p4,p5,p6],axis=1)
...
#             def energy_charge_discharge(model,t,v):
#                 return model.u1[t,v]=model.u1Positive[t,v]-model.u1Negative[t,v]
#             model.energy_charge_discharge=Constraint(model.t,mode.v,rule=energy_charge_discharge,doc='seperate charge and discharge')
...
#                     -sum([model.c1[t, v]*model.cb[t,v]*120*model.time_index for v in model.v])
#                     -sum([model.c2[t, v]*model.cb2[t,v]*120*model.time_index for v in model.v])
#                     for t in model.t] )
#                     #model.pr_ba
#                 model.objective = Objective(rule=objective_rule, sense=maximize, doc='Define objective function')
