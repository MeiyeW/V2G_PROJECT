...
df_gen = pd.read_csv('../data/netload/Generators.csv',header=0)
...
#Heat rate
model.heat_rate_ws = Param(model.WindSolar)
#Fuel cost
model.fuel_cost_ws = Param(model.WindSolar)
#Variable O&M
...
#Minmun down time
model.mindn_ws = Param(model.WindSolar)
...
def SysCost(model):
    ...
    sum(model.mwh_ws[j,i]*(model.heat_rate_ws[j]*model.fuel_cost_ws[j]+model.var_om_ws[j]) for i in model.hh_periods for j in model.WindSolar)+
    ...
    sum(model.regup_ws[j,i]*(model.heat_rate_ws[j]*model.fuel_cost_ws[j]+ model.var_om_ws[j])*0.5 for i in model.hh_periods for j in model.WindSolar)+
    ...
    sum(model.regdown_ws[j,i]*(model.heat_rate_ws[j]*model.fuel_cost_ws[j]+ model.var_om_ws[j])*0.3 for i in model.hh_periods for j in model.WindSolar)+
