...
model.Generators = model.GD1Gens
#  | model.GD2Gens | model.GD3Gens | model.GD4Gens | \
...
def SysCost(model):
    ...
    coal_st = sum(model.mwh[j,i]*(model.heat_rate[j]*5.2 + model.var_om[j]) for i in model.hh_periods for j in model.Coal_st)  
    oil_ic = sum(model.mwh[j,i]*(model.heat_rate[j]*6.0 + model.var_om[j]) for i in model.hh_periods for j in model.Oil_ic)
    oil_st = sum(model.mwh[j,i]*(model.heat_rate[j]*6.0 + model.var_om[j]) for i in model.hh_periods for j in model.Oil_st)

    imprt_v = sum(model.mwh[j,i]*65 for i in model.hh_periods for j in model.Imp_Viet)
    imprt_t = sum(model.mwh[j,i]*66 for i in model.hh_periods for j in model.Imp_Thai)
    import_hydro = sum(model.hydro_import[j,i]*48 for i in model.hh_periods for j in model.h_imports) 
