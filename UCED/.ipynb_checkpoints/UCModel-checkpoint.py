...
regup_margin = 0.15  ##minimum reserve as a percent of system demand
regdown_margin = 0.50 ##minimum spinning reserve as a percent of total reserve
...
### Minimum spinning reserve as a percent of total demand 
...
# model.mwh_price=Param(model.HH_periods, within=NonNegativeReals,initialize=0)
...
# model.regup_price=Var(model.HH_periods, within=NonNegativeReals,initialize=0)
...
# model.regdown_price=Var(model.HH_periods, within=NonNegativeReals,initialize=0)
...
def SysCost(model):
    ...
    #0.5 and 0.3 are the ROS for regulation up and down
    nonserved = sum(model.nse[i]*20 for i in model.hh_periods)
    #0.9 is the cost of non-served energy [$/MWh]
...
######========== Logical Constraint =========#############
# def mwh_price(model,i):
#     return model.mwh_price[i] == sum(((model.heat_rate[j]*model.fuel_cost[j]+model.var_om[j])*((model.mwh[j,i]>0)*1)) for j in model.Generators)
# model.mwh_priceContraint=Constraint(model.hh_periods,rule=mwh_price)
