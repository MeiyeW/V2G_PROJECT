...
class CentralOptimization(object):
    ...
    def solve(self, project, net_load, real_number_of_vehicle, SOC_margin=0.02,
        ...
        opti_model, result = self.process(self.times, self.vehicles, self.d, self.pmax,
                                          ...
                                          self.pr_e,self.pr_fre_c,self.pr_fre_p, self.pr_ba, self.timeindex, self.re_energy_r,
                                          peak_shaving, penalization)
        timer2 = time.time()
    ...
    def initialize_model(self, project, net_load, SOC_margin, SOC_offset,pr_e,pr_fre_c1,pr_fre_c2):
        ...
        self.pr_fre_c1.update()

        # Initialize pr_fre_c2
        self.pr_fre_c2.update()
    ...
    def process(self, times, vehicles, d, pmax, pmin, emin, emax, efinal, 
                pr_e, pr_fre_c, pr_fre_p, pr_ba, timeindex, re_energy_r,
                peak_shaving, penalization, solver="gurobi"):
        ...
        with SolverFactory(solver) as opt:
            ...
            # Regulation Energy Ratio
            model.re_energy_r = Param(initialize=0.25, doc='regulation energy ratio to capacity')
            ...
            if peak_shaving == 'economic':
                def objective_rule(model):
                    ...
                    +sum([model.c1[t, v]*model.pr_fre_c1*model.time_index for v in model.v])
                    +sum([model.c2[t, v]*model.pr_fre_c2*model.time_index for v in model.v])
                    +sum([model.c1[t, v]*model.pr_fre_c1*model.re_energy_u*model.time_index*model.re_energy_u for v in model.v])
                    +sum([model.c2[t, v]*model.pr_fre_c2*model.re_energy_d*model.time_index for v in model.v])
                    -sum(abs([model.u[t, v])*model.pr_ba*model.time_index for v in model.v]) for t in model.t])
