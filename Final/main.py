...
def saveResult(vehCap,pr,mwh_pd,myresult,versionName,day):
    ...
    pr.to_csv('../Result/'+versionName+"_"+str(day)+"_"+'_price.csv',index=False)
...
# # print(sys.argv[1:] )#['$1']
# # print(inputData[0])
# batteryCost= int(inputData[0])
# print('batteryCost:' + str(batteryCost))

# init sensitivity parametersbattery cost/ renewable portion/ vehicle numbers
batteryCost=14
renewablePercentage=0.39
solarWindMulti=(renewablePercentage-0.1163-0.0645)/(0.1163+0.0645)
vehicleNumber=1500000# 1.5 million BEVs in California 
versionName=str(yr)+'_'+str(run_no)+'_battery_'+str(batteryCost)+'_renewable_'+str(renewablePercentage)+ '_vehicleNum_'+str(vehicleNumber)
...
for day in range(daysInYear):
    ...
    while eps>150000 or eps2>50000:
        # # Load the load data
