...
def saveResult(pr,mwh_pd,myresult,vehicleSeperateResult,vehicles,versionName,day,month,path):
    ...
    myresult.to_csv(path+'/'+versionName+"_"+str(day)+'_vehicleGeneration.csv',index=False)
...
regulation=str(inputData[3])
month=str(inputData[4])
V1G=str(inputData[5])
...
versionName=str(year)+'_'+str(run_no)+'_battery_'+str(batteryCost)+'_renewable_'+str(renewablePercentage)+V1G
path='../Result/'+versionName+'/'+month
print(path)
if not os.path.exists('../Result/'+versionName):
    os.mkdir('../Result/'+versionName)
...
if year==2030:
    for i in range(20):#11GW more, I calculated the gap. 10% reserve. 
...
for day in dayRange: # 0,30;31,58;59,89
    ...
    while eps>vehicleNumber*0.05:
        ...
        print(eps)
        # print(eps2)    
    saveResult(pr[:24],mwh_pd,myresult,vehicleSeperateResult,project.vehicles,versionName,day,month,path)
