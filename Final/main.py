...
def saveResult(vehCap,pr,mwh_pd,myresult,versionName,day):
    vehCap.to_csv('../Result/'+versionName+"_"+str(day)+'_vehicleGeneration.csv',index=False)
    pr.to_csv('../Result/'+versionName+"_"+str(day)+'_price.csv',index=False)
    mwh_pd.to_csv('../Result/'+versionName+"_"+str(day)+'_Generator.csv',index=False)
    myresult.to_csv('../Result/'+versionName+"_"+str(day)+'_myresult.csv',index=False)
...
def init_charging(project):
    ...
    charging_stations.append(
        v2gsim.model.ChargingStation(name='L1_controllable', maximum_power=1400, minimum_power=-1400, post_simulation=True))
    charging_stations.append(
        v2gsim.model.ChargingStation(name='L2_controllable', maximum_power=7200, minimum_power=-7200, post_simulation=True))
    charging_stations.append(
        v2gsim.model.ChargingStation(name='L3_controllable', maximum_power=120000, minimum_power=-120000, post_simulation=True))
    charging_stations.append(    
    ...
    temp2 = pd.DataFrame(index=['L2_controllable'],
    ...
    for location in project.locations:
        ...
        location.available_charging_station = pd.concat([location.available_charging_station, temp2], axis=0)
...
home = pd.DataFrame(index=yearList, data={
    ...
    'L2_controllable': [0.175,0.1875, 0.2],
    'L2_V2G':[0.525,0.5625,0.6],
...
other_location = pd.DataFrame(index=yearList, data={ # changed other location
    'no_charger':[0.8,0.2, 0.6],
    'L1': [ 0.0,0.0, 0.0],
    'L2': [ 0.0, 0.0, 0.0],
    'L1_controllable': [ 0.0, 0.0, 0.0],
    'L2_controllable': [ 0.05, 0.1375, 0.075],
    'L2_V2G':[0.15,0.4125,0.225],
    'L3_controllable': [ 0.0, 0.25, 0.1]})
...
versionName=str(year)+'_'+str(run_no)+'_battery_'+str(batteryCost)+'_renewable_'+str(renewablePercentage)
...
for day in range(182,213,1):
    # daysRange=range(date,date+1)
    # for day in daysRange:# daysRange: #range(days)
    print(day+1)

    # if day==0: 
    #     vehCap=pd.read_csv('../Result/'+versionName+"_0_vehicleGeneration.csv",header=0)
    #     pr=pd.read_csv('../Result/'+versionName+"_0_price.csv",header=0)
    
    # else:
    #     vehCap=pd.read_csv('../Result/'+versionName+"_"+str(day-1)+'_vehicleGeneration.csv',header=0)
    #     pr=pd.read_csv('../Result/'+versionName+"_"+str(day-1)+'_price.csv',header=0)

    vehCap=pd.read_csv('../Result/2025_1_battery_10_renewable_0.4_0_vehicleGeneration.csv',header=0)
    pr=pd.read_csv    ('../Result/2025_1_battery_10_renewable_0.4_0_price.csv',header=0)
    ...
    while eps>vehicleNumber*0.05:
        ...
        print(eps)
        # print(eps2)    
    saveResult(vehCap[:24],pr[:24],mwh_pd,myresult,versionName,day)
    ...
    pd.DataFrame(data=revenueTotal,columns =['DemandCost', 'GenerationRevenue', 'RegUpRev','RegDownRev', 'RevenueTotal']).to_csv('../Result/'+versionName+"_"+str(day)+'_Revenue.csv',index=False)
    # gc.get_stats()